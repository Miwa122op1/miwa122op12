...
# tags: centos7,alma8,debian9,debian10,debian11,ubuntu1804,ubuntu2004,ubuntu2204,vzlinux8,rocky8,lic_ISPmanager_6_Lite
# revision: 24
...
#	sed -i"ispbak" -r "s/^(mirrorlist=)/#\1/g; s/^#(baseurl=)/\1/g" /etc/yum.repos.d/*.repo
	...
	curl -o install.sh "https://download.ispmanager.com/install.5.sh" || (sleep 1; curl -o install.sh "https://download.ispmanager.com/install.5.sh") || (sleep 1; curl -o install.sh "https://download.ispmanager.com/install.5.sh")
...
#	for file in /etc/yum.repos.d/*.repoispbak; do mv -f $file $(echo $file|sed 's/ispbak//'); done
	...
	wget -O install.sh "https://download.ispmanager.com/install.5.sh" || (sleep 1; wget -O install.sh "https://download.ispmanager.com/install.5.sh") || (sleep 1; wget -O install.sh "https://download.ispmanager.com/install.5.sh")
